...
RSpec.describe Item, type: :model do
  ...
  describe "Discount Percentage" do
     context 'when discount_percentage is negativ' do
      it { is_expected.to_not allow_value(-20).for(:discount_percentage) }
    end

     context 'when discount_percentage is superior to 100' do
      it { is_expected.to_not allow_value(200).for(:discount_percentage) }
  ...
  describe 'Original Price' do
    context 'when original price is negativ' do
      it { is_expected.to_not allow_value(-20).for(:original_price) }
    end

     context 'If original price is not present' do
